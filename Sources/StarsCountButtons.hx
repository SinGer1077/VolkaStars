...
class StarsCountButtons {
    ...
    public function draw(g: Graphics): Void{       
		...
		ui.begin(g);
        if (ui.window(Id.handle(), 10, 200, 240, 600, true)) {        
            ui.text("Stars count: "+GlobalState.starsCount);
            ui.text("(max:80k, min:0)");
            if (ui.button("+1000")){
                GlobalState.setStarsCount(1);
            }
            if (ui.button("-1000")){
                GlobalState.setStarsCount(-1);
            }
            if (ui.button("+10000")){
                GlobalState.setStarsCount(10);
            }
            if (ui.button("-10000")){
                GlobalState.setStarsCount(-10);
            }
